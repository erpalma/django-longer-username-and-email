...
def patch_user_model_first_name(model):
    field = model._meta.get_field("first_name")

    field.max_length = MAX_FIRST_NAME_LENGTH()
    field.help_text = _("Required, %s characters or fewer. Only letters, "
                        "numbers, and @, ., +, -, or _ "
                        "characters." % MAX_FIRST_NAME_LENGTH())

    # patch model field validator because validator doesn't change if we change
    # max_length
    for v in field.validators:
        if isinstance(v, MaxLengthValidator):
            v.limit_value = MAX_FIRST_NAME_LENGTH()


def patch_user_model_last_name(model):
    field = model._meta.get_field("last_name")

    field.max_length = MAX_LAST_NAME_LENGTH()
    field.help_text = _("Required, %s characters or fewer. Only letters, "
                        "numbers, and @, ., +, -, or _ "
